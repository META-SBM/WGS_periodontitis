...
# Define the variables to loop over
variables <- c("Stage_of_severity_of_periodontitis", 'Treatment_of_periodontitis', 'Antibiotics_treatment_in_last_6_months')

# Define contrasts for each variable
contrasts <- list(
  "Stage_of_severity_of_periodontitis" = c("Stage_of_severity_of_periodontitis", "severe", "mild"),
  "Treatment_of_periodontitis" = c("Treatment_of_periodontitis", "yes", "no"),
  'Antibiotics_treatment_in_last_6_months' = c('Antibiotics_treatment_in_last_6_months', 'yes', 'no')
)
...
for (variable in names(heatmap_data_des_list3)) {
  ...
  } else if (variable == "Antibiotics_treatment_in_last_6_months") {
    color_values <- c("yes" = "#C64742", "no" = "#01016f")
  } else {
...
# final plot w/log10 and abundance
...
color_schemes <- list(
  ...
  "Antibiotics_treatment_in_last_6_months"= c("yes" = "#C64742", "no" = "#01016f")
)
...
for(variable in names(heatmap_data_des_list3)) {
  ...
  if(is.null(ps_processed_list)) {
    ...
    p1 <- ggplot(plot_data, aes(x = reorder(Species, LogFC), y = LogFC, fill = Group)) +
      ...
      theme_minimal() +
      theme(
        axis.text.y = element_text(face = "italic", size = 16, colour = "black"),
